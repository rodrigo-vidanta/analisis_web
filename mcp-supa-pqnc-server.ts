...
class SupaPQNCServer {
  ...
  setupResourceHandlers() {
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => ({
      resources: [
        {
          uri: 'supabase://call_feedback',
          mimeType: 'application/json',
          name: 'Call Feedback',
          description: 'Retroalimentación de llamadas analizadas',
        },
        {
          uri: 'supabase://call_bookmarks',
          mimeType: 'application/json',
          name: 'Call Bookmarks',
          description: 'Marcadores de llamadas importantes',
        },
        {
          uri: 'supabase://system_preferences',
          mimeType: 'application/json',
          name: 'System Preferences',
          description: 'Preferencias del sistema por usuario',
        },
      ],
    }));
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      if (uri.startsWith('supabase://')) {
        ...
        let select = '*';
        if (table === 'pqnc_usuarios') {
          select = 'id,email,nombre,rol,activo,created_at';
        } else if (table === 'pqnc_roles') {
          select = 'id,nombre,permisos,descripcion';
        } else if (table === 'pqnc_ejecutivos') {
          select = 'id,nombre,email,extension,equipo,activo';
        } else if (table === 'pqnc_analisis') {
          select = 'id,call_id,score,feedback,created_at';
        } else if (table === 'pqnc_feedback') {
          select = 'id,analisis_id,tipo,mensaje,created_at';
        }
  ...
  async queryTable(args) {
    ...
    // Debug log
    console.error(`[SupaPQNC] queryTable: table=${table}, url=${SUPABASE_URL}`);
    ...
    console.error(`[SupaPQNC] queryTable result: error=${error?.message || 'none'}, rows=${data?.length || 0}`);
  ...
  async run() {
    ...
    console.error('SupaPQNC MCP server running on stdio');
  }
}

const server = new SupaPQNCServer();
