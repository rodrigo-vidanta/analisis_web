-- ============================================
-- SISTEMA COMPLETO DE PARÁFRASIS Y MODERACIÓN
...
-- ============================================
-- 1. TABLA DE LOGS DE PARÁFRASIS (TODOS LOS INPUTS/OUTPUTS)
-- ============================================
...
-- ============================================
-- 2. TABLA DE WARNINGS DE MODERACIÓN (ya existe, pero la mantenemos)
-- ============================================
...
-- ============================================
-- 3. TABLA DE CONTADOR DE WARNINGS (OPTIMIZADA)
-- ============================================
...
-- Función para registrar un warning de moderación
...
BEGIN
  SELECT COALESCE(is_blocked, false) INTO v_is_blocked
  FROM user_warning_counters
  WHERE user_id = p_user_id;
  
  -- Si no existe registro, no está bloqueado
...
-- Vista para estadísticas de parafraseo por usuario
...
-- Política: Usuarios pueden ver sus propios warnings
...
-- Políticas para admins (si existe estructura de auth)
DO $$
BEGIN
  IF EXISTS (
    ...
    DROP POLICY IF EXISTS "Admins can view all paraphrase logs" ON paraphrase_logs;
    CREATE POLICY "Admins can view all paraphrase logs"
      ON paraphrase_logs
      FOR SELECT
      USING (
        EXISTS (
    ...
    -- Admins pueden ver todos los warnings
    ...
    -- Admins pueden ver todos los contadores
    DROP POLICY IF EXISTS "Admins can view all warning counters" ON user_warning_counters;
    CREATE POLICY "Admins can view all warning counters"
      ON user_warning_counters
      FOR SELECT
      USING (
        EXISTS (
...
COMMENT ON FUNCTION update_user_warning_counter IS 'Trigger que actualiza automáticamente el contador cuando se inserta un warning';

-- Función para resetear warnings (desbloquear usuario)
