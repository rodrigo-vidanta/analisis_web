...
|--------|-------|-------------|
| **Content-Security-Policy** | `default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://*.supabase.co https://*.vidavacations.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https: blob:; connect-src 'self' https://*.supabase.co https://glsmifhkoaifvaegsozd.supabase.co https://*.vidavacations.com wss://*.supabase.co wss://*.vapi.ai; frame-src 'self' https://*.supabase.co;` | Controla qué recursos puede cargar el navegador |
...
- El CSP está configurado para permitir recursos de Supabase, vidavacations.com y VAPI (WebSocket para escuchar llamadas)
