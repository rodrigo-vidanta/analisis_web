/**
 * ============================================
 * COMPONENTE PRINCIPAL - LIVE CHAT COMPLETE
 * ============================================
 *
...
const LiveChatComplete: React.FC = () => {
  ...
  const [columnWidths, setColumnWidths] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : { conversations: 30, blocks: 25, chat: 45 };
  });
      ...
      <div className="fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg p-2 shadow-lg">
        <div className="flex items-center space-x-2">
          ...
          <input
            ...
            min="20"
            max="50"
            value={columnWidths.conversations}
            onChange={(e) => {
              const newWidth = parseInt(e.target.value);
              const remaining = 100 - newWidth;
              setColumnWidths({
                conversations: newWidth,
                blocks: Math.round(remaining * 0.4),
                chat: Math.round(remaining * 0.6)
              });
            }}
            className="w-20"
          />
...
export default LiveChatComplete;
