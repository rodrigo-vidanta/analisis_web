...
const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ forceMode }) => {
  ...
  const [selectedAnalysis, setSelectedAnalysis] = useState<'natalia' | 'pqnc'>(() => {
    ...
    if (user.role_name === 'admin' || user.role_name === 'developer') return 'natalia';
    return 'natalia';
  });

  // Ajustar selección para evaluadores una vez que los permisos estén disponibles
  useEffect(() => {
    if (forceMode || !user || user.role_name !== 'evaluator') return;
    try {
      if (canAccessSubModule('pqnc')) {
        setSelectedAnalysis('pqnc');
      }
    } catch {
      // Permisos aún no disponibles, mantener default
    }
  }, [user, forceMode, canAccessSubModule]);
