...
const MyAgents: React.FC<MyAgentsProps> = ({ onOpenEditor }) => {
  ...
  const EmptyState: React.FC<{ title: string; subtitle: string }> = ({ title, subtitle }) => (
    <div className="text-center py-12 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl bg-slate-50/50 dark:bg-slate-800/50">
      ...
      <h4 className="text-slate-700 dark:text-slate-300 font-medium mb-1">{title}</h4>
    ...
    <div className="space-y-6">
      <div className="glass-card p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-1">Mis Agentes</h3>
            <p className="text-sm text-slate-600 dark:text-slate-400">Borradores y publicados creados por ti</p>
          </div>
          <button onClick={loadAgents} className="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700">
            Recargar
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h4 className="text-lg font-semibold text-slate-900 dark:text-white">En progreso</h4>
        </div>
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {drafts.map(a => (
              <div key={a.id} className="border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h5 className="font-semibold text-slate-900 dark:text-white line-clamp-1">{a.name}</h5>
                    <p className="text-xs text-slate-500">{a.estimated_time || ''}</p>
                  </div>
                  <span className="px-2 py-1 text-xs rounded-full bg-amber-100 text-amber-700">Borrador</span>
                </div>
                <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 mb-3">{a.description || 'Agente en edición'}</p>
      ...
      <div className="glass-card p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h4 className="text-lg font-semibold text-slate-900 dark:text-white">Publicados</h4>
        </div>
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {published.map(a => (
              <div key={a.id} className="border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 p-4">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 mb-3">{a.description || 'Sin descripción'}</p>
                <div className="flex items-center justify-end gap-2">
