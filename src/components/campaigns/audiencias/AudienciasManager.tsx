                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {audience.dias_sin_contacto && audience.dias_sin_contacto > 0 && (
                              <span className="px-2 py-0.5 text-xs bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded flex items-center gap-1">
                                <Clock className="w-3 h-3" />
                                {audience.dias_sin_contacto}d
                              </span>
                            )}
                            ...
                            {!audience.destinos?.length && !audience.viaja_con?.length && !audience.estado_civil && !audience.dias_sin_contacto && (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {formData.dias_sin_contacto && formData.dias_sin_contacto > 0 && (
                    <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                      Sin actividad en {formData.dias_sin_contacto} días
                    </span>
                  )}
                </div>
                
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Incluir prospectos con más de X días sin ser contactados
                </p>
                
                {/* Presets */}
                <div className="flex flex-wrap gap-2">
                  {DIAS_SIN_CONTACTO_PRESETS.map((preset) => {
                    const isSelected = formData.dias_sin_contacto === preset.value;
                    return (
                      <button
                        key={preset.value}
                        type="button"
                        onClick={() => {
                          if (isSelected) {
                            setFormData({ ...formData, dias_sin_contacto: null });
                            setCustomDays('');
                          } else {
                            setFormData({ ...formData, dias_sin_contacto: preset.value });
                            setCustomDays(preset.value.toString());
                          }
                        }}
                        className={`px-3 py-2 rounded-lg border-2 transition-all flex flex-col items-center ${
                          isSelected
                            ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/30'
                            : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                        }`}
                      >
                        <span className={`text-sm font-bold ${isSelected ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-gray-300'}`}>
                          {preset.label}
                        </span>
                        <span className={`text-[10px] ${isSelected ? 'text-amber-500 dark:text-amber-500' : 'text-gray-500 dark:text-gray-400'}`}>
                          {preset.description}
                        </span>
                      </button>
                    );
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (numValue > 0 && numValue <= 365) {
                          setFormData({ ...formData, dias_sin_contacto: numValue });
                        } else if (!value) {
                          setFormData({ ...formData, dias_sin_contacto: null });
                        }
                      }}
                      className="w-full pl-10 pr-16 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-gray-800/50 dark:text-white"
                    />
                    <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-gray-400">
                      días
                    </span>
                  </div>
                  {formData.dias_sin_contacto && (
                    <button
                      type="button"
                      onClick={() => {
                        setFormData({ ...formData, dias_sin_contacto: null });
                      ...
                      className="p-2.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
  ...
  return (
    ...
    >
      ...
      <div className="p-4 pt-6">
        ...
        {/* Tags de criterios */}
        {((audience.destinos && audience.destinos.length > 0) || (audience.viaja_con && audience.viaja_con.length > 0) || audience.estado_civil || audience.dias_sin_contacto) && (
          <div className="mb-3 space-y-1">
            <div className="flex flex-wrap gap-1">
              {audience.dias_sin_contacto && audience.dias_sin_contacto > 0 && (
                <span className="px-2 py-0.5 text-[9px] font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-md flex items-center gap-1">
                  <Clock className="w-2.5 h-2.5" />
                  {audience.dias_sin_contacto}+ días
                </span>
              )}
              {audience.destinos && audience.destinos.length > 0 && (
          ...
          <div className="flex items-center gap-3 text-[10px] text-gray-500 dark:text-gray-400">
            {audience.dias_sin_contacto && audience.dias_sin_contacto > 0 && (
              <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400">
                <Clock className="w-3 h-3" />
                {audience.dias_sin_contacto}d sin contacto
              </span>
            )}
            {audience.destinos && audience.destinos.length > 0 && (
