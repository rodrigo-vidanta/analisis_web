...
const TimelineCard: React.FC<TimelineCardProps> = memo(({
  ...
  if (showArchivedView) {
    return (
      ...
      >
        {/* Línea conectora */}
        <div className="absolute left-0 top-0 bottom-0 w-px bg-gray-200 dark:bg-white/10 group-hover:bg-gray-300 dark:group-hover:bg-white/20 transition-colors">
           <div className="absolute top-6 -left-[5px] w-2.5 h-2.5 rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-white/30 group-hover:border-gray-400 dark:group-hover:border-white/60 transition-colors" />
        </div>

        <div className="relative overflow-hidden rounded-2xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 p-4 hover:bg-gray-50 dark:hover:bg-white/10 transition-all duration-300 shadow-sm dark:shadow-none">
           <div className="flex justify-between items-start">
              <div>
                 <h3 className="text-sm font-medium text-gray-500 dark:text-white/70 line-through">{activity.title}</h3>
                 <p className="text-xs text-gray-400 dark:text-white/40 mt-1">{new Date(activity.due_date).toLocaleDateString()}</p>
              </div>
              {onUnarchive && (
    ...
    >
      ...
      {!hideDot && (
        <div className={cn(
          ...
          dotClass,
          "bg-gray-50 dark:bg-[#0f172a]", // Fondo del punto
          isSelected 
            ...
            : activity.realizado 
              ...
              : activity.priority === 'critica' 
                ...
                : "border-gray-300 dark:border-white/20 group-hover:border-cyan-400 dark:group-hover:border-cyan-400 group-hover:shadow-[0_0_8px_rgba(34,211,238,0.5)] group-hover:scale-125" // Hover azul neón
        )} />
      )}
      ...
      <div 
        ...
        className={cn(
          ...
          isSelected 
             ...
             : "bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-white/20 shadow-sm dark:shadow-none"
        )}
      >
         {/* Decoración Gradient en Hover */}
         <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-gray-100/50 to-transparent dark:from-white/5 dark:to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
         ...
         <div className={cn("flex items-start gap-4", isLeft ? 'flex-row-reverse' : 'flex-row')}>
            ...
            <button
               ...
               className={cn(
                 ...
                 activity.realizado 
                   ...
                   : "text-gray-300 dark:text-white/20 hover:text-cyan-400 dark:hover:text-cyan-400 hover:bg-gray-100 dark:hover:bg-white/10"
               )}
            >
            ...
            <div className="flex-1 min-w-0">
               <h3 className={cn(
                 ...
                 activity.realizado 
                   ? "text-gray-400 dark:text-white/40 line-through decoration-gray-300 dark:decoration-white/20" 
               ...
               <div className={cn("flex flex-wrap items-center gap-2", isLeft ? 'justify-end' : 'justify-start')}>
                  {/* Priority Badge */}
                  <span 
                    className="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border backdrop-blur-sm"
                    style={{
                  ...
                  >
                    <span className="text-gray-700 dark:text-white">{activity.priority}</span>
                  </span>

                  {/* Attachments Indicator */}
                  {activity.attachments && activity.attachments.length > 0 && (
                     <div className="px-2 py-1 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 flex items-center gap-1.5 text-[10px] text-gray-500 dark:text-white/50">
                  ...
                  {activity.asignado_a && activity.asignado_a.length > 0 && (
                     <div className="flex -space-x-2">
                        {activity.asignado_a.slice(0, 3).map((name, i) => (
                           <div key={i} className="w-6 h-6 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-900 border border-white dark:border-white/10 flex items-center justify-center text-[9px] text-gray-700 dark:text-white/90 shadow-sm z-10 cursor-help" title={name}>
                              {name.charAt(0).toUpperCase()}
                           </div>
                        ))}
                        {activity.asignado_a.length > 3 && (
                           <div className="w-6 h-6 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center text-[9px] text-gray-500 dark:text-white/50 z-0" title={`${activity.asignado_a.length - 3} más`}>
                              +{activity.asignado_a.length - 3}
                           </div>
                        )}
                     </div>
         ...
         <div className={cn("absolute top-4 flex flex-col gap-2 transition-all duration-300 opacity-0 group-hover:opacity-100 z-30 pointer-events-auto", isLeft ? 'left-4' : 'right-4')}>
            <button
               ...
               className="p-2 rounded-full bg-gray-200/80 dark:bg-black/40 text-gray-600 dark:text-white/70 hover:text-gray-900 dark:hover:text-white hover:bg-gray-300 dark:hover:bg-black/60 backdrop-blur-sm shadow-lg"
               title="Más opciones"
            >
